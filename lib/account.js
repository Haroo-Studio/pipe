...
exports.createByEmailFromWeb = function (params, callback) {
    var req = params['req'];

    Account.findOne({ email: params['email'] }, function(err, existingUser) {
        if (err) {
            return callback(err);
        }
        ...
        if (existingUser) {
            ...
            callback({ msg: 'Account with that email address already exists.' }, existingUser);
        } else {
            var user = new Account({
                email: params['email'],
                password: params['password'],
...
exports.passwordResetByEmailFromWeb = function (params, callback) {
    Account.findOne({email: params['email']}, function (err, existAccount) {
        if (existAccount && existAccount.email) {
            var randomToken = CommonUtil.getRandomToken();
            existAccount.reset_password_token = randomToken;
            ...
            callback(err, existAccount);
        } else {
            callback({msg: "No email information existed"});
        }
    });
};
