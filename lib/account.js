...
exports.createByEmail = function (params, callback) {
    Account.findOne({ email: params['email'] }, function(err, existUser) {
        if (err) {
            params['result'] = CommonUtil.setPassportErrorToClient(err, HarooCode.account.create.database, params['result']);
        ...
        if (existUser) {
            ...
            var user = new Account({
                ...
                haroo_id: CommonUtil.initHarooID(params['email']),
    ...
    passport.authenticate('local', function(err, loginUser, info) {
        ...
        if (loginUser && loginUser._id) {
            Account.findById(loginUser._id, function (err, updateUser) {
                ...
                token.save(function (err) {
                    if (err) {
                        params['result']= HarooCode.token.database;
    ...
    Account.findOne({access_token: params['accessToken']}, function (err, existUser) {
        ...
        if (existUser) {
            ...
            if (existUser.login_expire > now) {
                ...
                // remove access token
                existUser.access_token = undefined;
                existUser.login_expire = undefined;
                ...
                existUser.save(function (err) {
                    ...
                    params['result'] = CommonUtil.setAccountToClient(HarooCode.account.token.expired, existUser);
...
exports.deleteByPassport = function (params, callback) {
    ...
    passport.authenticate('local', function(err, validUser, info) {
        ...
        if (validUser && validUser.access_token == params['accessToken']) {
            // expired?
            var now = Date.now();
            ...
            if (validUser.login_expire > now) {
                ...
                params['result'] = HarooCode.account.haroo_id.token_expired;
